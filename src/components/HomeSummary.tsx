...
const INVESTMENT_STORAGE_KEY = "household-expense-demo-investments";
  ...
  return [
    {
      ...
      expenses: [
        ...
        { date: "2026-06-07", category: "生活用品", item: "全聯", amount: 780, note: "衛生紙、洗碗精", paymentMethod: "信用卡", creditCard: viewer === "chris" ? "玉山" : "台新" },
    ...
    },
    {
      ...
      expenses: [
        { date: "2026-06-02", category: "生活用品", item: "生活用品", amount: 1200, note: "清潔用品", paymentMethod: "信用卡", creditCard: viewer === "chris" ? "台新" : "玉山" },
    ...
    },
    {
      ...
      expenses: [
        ...
        { date: "2026-06-05", category: "保健食品", item: "保健食品", amount: 1800, note: "兒童益生菌", paidBy: viewer === "chris" ? "我" : "先生", paymentMethod: "信用卡", creditCard: viewer === "chris" ? "玉山" : "台新" },
    ...
    },
    {
      ...
      expenses: [
        ...
        { date: "2026-05-20", category: "寵物", item: "貓砂", amount: 520, note: "貓砂", paymentMethod: "信用卡", creditCard: viewer === "chris" ? "玉山" : "台新" },
      ],
    },
  ];
}
...
export function HomeSummary({ viewer }: { viewer: Viewer }) {
  ...
  const [investments, setInvestments] = useState<InvestmentRecord[]>(loadInvestments);
  ...
  useEffect(() => {
    ...
    window.addEventListener("investment-records-updated", refreshInvestments);
    window.addEventListener("storage", refreshInvestments);
    refreshInvestments();
    return () => {
      window.removeEventListener("investment-records-updated", refreshInvestments);
  ...
  const visibleInvestments = scope === "all" ? investments : investments.filter((record) => record.date.startsWith(selectedMonth));
  const investmentTotal = visibleInvestments.reduce((sum, record) => sum + record.amount, 0);
    ...
    <section className="grid">
      ...
      <article className="card grid">
        ...
        >
          ...
          <strong>{money(investmentTotal)}</strong>
        </button>
        {showInvestments ? (
          <div className="grid">
            {visibleInvestments.length === 0 ? <p className="muted">目前沒有投資紀錄</p> : null}
            {visibleInvestments.map((record) => (
              <div className="row" key={record.id}>
                <span>{record.date.slice(5)}　{record.type}・{record.name}</span>
